...
A CLI for exploring and reading content from a Notion workspace. Built using [Postman Code](https://github.com/postmanlabs/postman-code-examples) to generate type-safe API clients from the official [Notion API Postman Collection](https://www.postman.com/notionhq/notion-s-api-workspace/collection/a1b2c3d/notion-api).

The CLI covers workspace exploration — searching for pages, reading content, querying databases, and navigating the page hierarchy. It doesn't cover the full Notion API (no creating, updating, or deleting content). Designed for humans and AI agents that need to find and read things in Notion.

## What It Does

- **Search** — Find pages by keyword or list all workspace-level root pages
- **Read Pages** — Fetch a page's content, properties, and child pages/databases
- **Query Databases** — View a database's schema and list its entries
- **Navigate** — Traverse the workspace tree from roots to leaves

## How It Works

The API clients in `src/postman/notion-api/` were generated by [Postman Code](https://www.postman.com/explore/code), which turns requests in a Postman Collection into type-safe TypeScript functions. Each request in the [Notion API Collection](https://www.postman.com/notionhq/notion-s-api-workspace/collection/a1b2c3d/notion-api) becomes its own `client.ts` file with typed parameters, response types, and error handling — no hand-written fetch calls needed.

The generated clients are re-exported through an `index.ts` barrel file that namespaces them by resource, so using them feels like calling an SDK:
...
The `--workspace` flag paginates through all results internally and returns only pages at the workspace level — these are your starting points for mapping a workspace.
...
│   ├── index.ts                     # SDK-style barrel export
