...
A CLI for exploring Notion workspaces. Search for pages, read their content, and query databases.
...
Those generated clients are re-exported through an `index.ts` barrel file that namespaces them by resource, so calling them feels like using an SDK:
